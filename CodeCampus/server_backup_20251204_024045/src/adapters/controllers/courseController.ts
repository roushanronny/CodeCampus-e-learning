...
const courseController = (
  ...
  const getLessonsByCourse = asyncHandler(
    async (req: Request, res: Response) => {
      ...
      // Transform lessons to include thumbnail and videoUrl from media array
      // Only transform if media exists, and handle errors gracefully
      const transformedLessons = (lessons || []).map((lesson: any) => {
        const transformedLesson = lesson.toObject ? lesson.toObject() : { ...lesson };
        
        // Extract thumbnail and video from media array
        if (transformedLesson.media && Array.isArray(transformedLesson.media)) {
          const thumbnailMedia = transformedLesson.media.find((m: any) => 
            m.name?.toLowerCase().includes('thumbnail') || 
            m.name?.toLowerCase().includes('image') ||
            m.key?.toLowerCase().includes('thumbnail')
          );
          const videoMedia = transformedLesson.media.find((m: any) => 
            m.name?.toLowerCase().includes('video') || 
            m.name?.toLowerCase().includes('lessonvideo') ||
            m.key?.toLowerCase().includes('video')
          );
          
          // Set thumbnail URL - use local URL directly for development
          if (thumbnailMedia?.key) {
            transformedLesson.thumbnail = `http://localhost:${process.env.PORT || 4000}/uploads/${thumbnailMedia.key}`;
          }
          
          // Set video URL - use local URL directly for development
          if (videoMedia?.key) {
            transformedLesson.videoUrl = `http://localhost:${process.env.PORT || 4000}/uploads/${videoMedia.key}`;
          }
        }
